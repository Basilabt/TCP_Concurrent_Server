...
#define MAX_ALLOWED_CONCURRENT_CLIENTS 4
...
void serveClient(const int connfd , const struct sockaddr_in clientInfo) {
    ...
    while( (numberOfBytes = read(connfd,rcvLine,BUFFER_SIZE)) > 0 ) {
        ...
        switch (clientOption) {
            ...
            case 5: {
                char *answer = findRepeatedWords(line);
                snprintf(result, sizeof(result), "OP5(%s)=%s", line, answer);
...
char* findRepeatedWords(const char* inputString) {
    static char result[MAX_WORDS * MAX_WORD_LENGTH];
    char words[MAX_WORDS][MAX_WORD_LENGTH];
    int wordCount = 0, charIndex = 0, i, j, duplicateCount;
    result[0] = '\0';

    for (int k = 0; inputString[k] != '\0'; k++) {
        if (inputString[k] != ' ' && inputString[k] != '\0') {
            words[wordCount][charIndex++] = inputString[k];
        } else {
            if (charIndex > 0) {
